...
async function runOnce() {
  ...
  const processOne = async (L) => {
    ...
    try {
      ...
      const html = renderHtml(name, L.department, w.dateLabel, L.enterprise_id, L.team_id, w.localDate, tz, metricsFull, camps);
  ...
  async function worker(L) {
    ...
    for (const day of days) {
      ...
      try {
        ...
        const html = renderHtml(name, L.department, w.dateLabel, L.enterprise_id, L.team_id, day, tz, metrics, camps);
...
async function generateAndSendNow(opts) {
  ...
  const [liveRes, cfgRes, recRes] = await Promise.all([
    sb.from("roi_live_departments").select("team_id,enterprise_id,department,dry_run").eq("is_live", true),
    sb.from("roi_rooftop_config").select("team_id,rooftop_name,timezone,digest_send_hour"),
    sb.from("roi_recipients").select("team_id,email,receives_sales,receives_service,email_enabled"),
  ]);
  ...
  const process1 = async (L) => {
    ...
    try {
      ...
      const metricsFull = { ...metrics, campaigns: camps, appointments: enr.appointments, topVehicles: enr.topVehicles, dollarRate };
      const html = renderHtml(name, L.department, w.dateLabel, L.enterprise_id, L.team_id, w.localDate, tz, metricsFull, camps, cadence);
...
async function previewDigestNow(opts) {
  ...
  const [liveRes, cfgRes] = await Promise.all([
    ...
    sb.from("roi_rooftop_config").select("team_id,rooftop_name,timezone").eq("team_id", teamId).maybeSingle(),
  ]);
  ...
  const metricsFull = { ...metrics, campaigns: camps, appointments: enr.appointments, topVehicles: enr.topVehicles, dollarRate };
  const html = renderHtml(name, department, w.dateLabel, enterpriseId, teamId, w.localDate, tz, metricsFull, camps, cadence);
...
async function runCadence(cadence) {
  ...
  const [liveRes, cfgRes, recRes] = await Promise.all([
    sb.from("roi_live_departments").select("team_id,enterprise_id,department,dry_run").eq("is_live", true),
    sb.from("roi_rooftop_config").select(`team_id,rooftop_name,timezone,digest_send_hour,daily_enabled,${enabledCol}`),
    sb.from("roi_recipients").select("team_id,email,receives_sales,receives_service,email_enabled"),
  ]);
  if (liveRes.error || cfgRes.error || recRes.error) throw new Error((liveRes.error || cfgRes.error || recRes.error).message);
  const cfgOf = new Map((cfgRes.data ?? []).map((c) => [c.team_id, c]));
...
module.exports = { runOnce, runCadence, generateAndSendNow, previewDigestNow, backfill, rerender, renderHtml, sendMail, syncLive, apiMetrics, apiActionItems, apiCampaigns };
