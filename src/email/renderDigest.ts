...
export function renderDigestEmail(metrics: DigestMetrics, opts: RenderOpts): string {
  ...
  // total conversations (hero) + inbound-only split (channel breakdown)
  const call = n(m.conversationsCall), sms = n(m.conversationsSms), chat = n(m.conversationsChat);
  const callIn = n(m.conversationsCallIn), smsIn = n(m.conversationsSmsIn), chatIn = n(m.conversationsChatIn);
  const callOut = n(m.conversationsCallOut), smsOut = n(m.conversationsSmsOut), chatOut = n(m.conversationsChatOut);
  ...
  const camps = ((Array.isArray(m.campaigns) ? m.campaigns : []) as Array<{ name: string; dials: number; appts: number; conversion: string | number }>).filter((c) => n(c.dials) > 0); // drop zero-dial campaigns
